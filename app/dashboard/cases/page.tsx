...
export default async function CasesPage(props: {
  ...
  const customStart =
    searchParams.range === "custom" ? searchParams.startDate : undefined;
  const customEnd =
    searchParams.range === "custom" ? searchParams.endDate : undefined;
  ...
  const { data: casesData, error } = await supabase.rpc("search_cases", {
    p_query: query,
  ...
  if (error) {
    // Error occurred while fetching cases
    redirect("/dashboard?error=Failed to load cases");
  }
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-4 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400 max-w-2xl">
            Manage and track all reported cases. Use the filters below to
