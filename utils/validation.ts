...
export const createCaseSchema = z.object({
  ...
  narrative_facts: z
    .string()
    .min(10, "Please provide more details in the facts"),
...
export const addInvolvedPartySchema = z.object({
  name: z
    .string()
    .min(
      CONFIG.INVOLVED_PARTY.NAME_MIN_LENGTH,
      `Name must be at least ${CONFIG.INVOLVED_PARTY.NAME_MIN_LENGTH} characters`,
    )
    .max(
      CONFIG.INVOLVED_PARTY.NAME_MAX_LENGTH,
      `Name must be less than ${CONFIG.INVOLVED_PARTY.NAME_MAX_LENGTH} characters`,
    ),
  ...
  address: z
    .string()
    .max(
      CONFIG.INVOLVED_PARTY.ADDRESS_MAX_LENGTH,
      `Address is too long (max ${CONFIG.INVOLVED_PARTY.ADDRESS_MAX_LENGTH} characters)`,
    )
    .optional()
    .or(z.literal("")),
});
...
export const addCaseNoteSchema = z.object({
  content: z
    ...
    .max(
      CONFIG.CASE_NOTE.MAX_LENGTH,
      `Note is too long (max ${CONFIG.CASE_NOTE.MAX_LENGTH} characters)`,
    ),
});
...
export const passwordSchema = z
  ...
  .min(
    ...
    `Password must be at least ${CONFIG.PASSWORD.MIN_LENGTH} characters`,
  )
...
export const guestLinkDurationSchema = z
  .number()
  .int("Duration must be a whole number")
  .min(
    CONFIG.GUEST_LINK.MIN_DURATION_HOURS,
    `Duration must be at least ${CONFIG.GUEST_LINK.MIN_DURATION_HOURS} hour`,
  )
  .max(
    CONFIG.GUEST_LINK.MAX_DURATION_HOURS,
    `Duration cannot exceed ${CONFIG.GUEST_LINK.MAX_DURATION_HOURS} hours`,
  );
