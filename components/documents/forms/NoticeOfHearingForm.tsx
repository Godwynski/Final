...
export default function NoticeOfHearingForm({ caseData, complainants, respondents, settings, hearings = [] }: NoticeOfHearingFormProps) {
    ...
    if (latestHearing) {
        ...
        time = date.toLocaleTimeString('default', { hour: 'numeric', minute: '2-digit', hour12: true })
    }
        ...
        <div className="max-w-[210mm] mx-auto p-8 bg-white min-h-screen text-black font-serif">
            ...
            <div className="outline-none" contentEditable suppressContentEditableWarning>
                ...
                <p className="mb-4 text-justify indent-8 leading-relaxed">
                    ...
                    <span className="inline-block w-32 border-b border-black mx-1 text-center font-bold px-2">{day}</span> day of
                    <span className="inline-block w-32 border-b border-black mx-1 text-center font-bold px-2">{monthYear}</span>, 20__, at
                    <span className="inline-block w-24 border-b border-black mx-1 text-center font-bold px-2">{time}</span> o'clock in the morning/afternoon,
