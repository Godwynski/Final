...
export default function DashboardCalendar() {
  ...
  const selectedHearings = selectedDate
    ? getHearingsForDay(selectedDate.getDate())
    : [];
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm dark:border-gray-700 dark:bg-gray-800 flex flex-col h-full">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1">
          <button
            onClick={prevMonth}
            className="p-1.5 hover:bg-white rounded-md shadow-sm dark:hover:bg-gray-600 transition-all text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white"
          >
      ...
      <div className="p-4 flex-grow">
        <div className="grid grid-cols-7 gap-1 mb-2">
          {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((day) => (
            <div
              key={day}
              className="text-center text-[10px] uppercase font-bold text-gray-400 py-1 tracking-wider"
            >
              {day}
            </div>
          ))}
                ...
                {hasHearings && (
                  <div className="mt-1 flex gap-0.5">
                    {dayHearings.slice(0, 3).map((h, idx) => {
                      const colors = getStatusColor(h.cases.status);

                      return (
                        <div
                          key={idx}
                          className={`w-1.5 h-1.5 rounded-full ${isToday ? "bg-white" : colors.dot}`}
                        />
                      );
                    })}
                  </div>
                  ...
                  {selectedHearings.map((h) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <span
                          className={`text-xs font-bold px-2 py-0.5 rounded ${getStatusColor(h.cases.status).badge}`}
                        >
                          {new Date(h.hearing_date).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </span>
                        <Link
                          href={`/dashboard/cases/${h.cases.id}`}
