...
export default function ResolutionBanner({
    ...
    const reason = resolutionDetails?.reason || resolutionDetails?.agency || resolutionDetails?.terms || 'No details recorded.'
    const officer = resolutionDetails?.officer || 'Unknown Officer'
    const date = formattedDate || (resolutionDetails?.date ? new Date(resolutionDetails.date).toISOString().split('T')[0] : 'Unknown Date')
        ...
        <div className={`mb-6 p-6 rounded-lg border ${bgColor} shadow-sm`}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-start gap-4">
                    <div className={`p-3 rounded-full bg-white dark:bg-gray-800 shadow-sm ${textColor}`}>
                        {/* Simple Icon Placeholder */}
                        <span className="text-2xl font-bold">
                            {type === 'Dismissed' ? '✕' : type === 'Referred' ? '→' : type === 'Settled' ? '✓' : 'Archive'}
                        </span>
                    </div>
                    <div>
                        <h2 className={`text-xl font-bold ${textColor} uppercase tracking-wide`}>{title}</h2>
                        <p className="mt-1 text-base font-medium text-gray-700 dark:text-gray-300">
                            {type === 'Referred' ? 'Agency:' : 'Reason/Terms:'} <span className="font-normal">{reason}</span>
                        </p>
                        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                            Action taken by <span className="font-semibold">{officer}</span> on {date}
                        </p>
                    </div>
                </div>
