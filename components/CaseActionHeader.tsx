...
export default function CaseActionHeader({ status, caseId, hearings = [] }: { status: string, caseId: string, hearings?: any[] }) {
    ...
    const actions = rawActions.filter(action => {
        ...
        if (action.action === 'schedule_hearing') {
            const hasActiveHearing = hearings.some(h => ['Scheduled', 'Rescheduled'].includes(h.status))
    ...
    const handleActionClick = async (action: WorkflowAction) => {
        if (action.requiresInput) {
            ...
            if (action.action === 'reschedule_hearing') {
                const activeHearing = hearings.find(h => ['Scheduled', 'Rescheduled'].includes(h.status))
                if (activeHearing) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (['schedule_hearing', 'reschedule_hearing'].includes(selectedAction.action)) {
            finalInput = JSON.stringify({ date: inputVal, type: hearingType })
        }

        const res = await performCaseAction(caseId, selectedAction.action, finalInput)
        setLoading(false)
                                ...
                                {['schedule_hearing', 'reschedule_hearing'].includes(selectedAction.action) ? (
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-1 gap-2">
                                                {Object.entries(HEARING_TYPES).map(([type, desc]) => (
                                                    <div key={type} className={`relative flex items-start p-3 rounded-lg border cursor-pointer transition-all ${hearingType === type ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800'}`} onClick={() => setHearingType(type)}>
                                                        ...
                                                        <div className="ml-3 text-sm">
                                                            <label className={`font-medium ${hearingType === type ? 'text-blue-700 dark:text-blue-400' : 'text-gray-900 dark:text-gray-300'}`}>
                                                                {type}
                                                            </label>
                                    ...
                                    <>
                                        ...
                                        {selectedAction.inputType === 'textarea' ? (
                                            ...
                                            <input
                                                type={selectedAction.inputType === 'datetime-local' ? 'datetime-local' : selectedAction.inputType}
                                                required
                                                value={inputVal}
                                                onChange={(e) => setInputVal(e.target.value)}
                                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
